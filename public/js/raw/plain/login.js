...
var timeSince = function timeSince(date) {
  if ((typeof date === "undefined" ? "undefined" : typeof date) !== "object") {
    date = new Date(date);
  }
  ...
  navigator.serviceWorker.ready.then(function(registration) {
    ...
    registration.pushManager
      ...
      .then(function(subscription) {
        window.sub = subscription;
        /*console.log(
          JSON.parse(localsave("save-checked", "get")),
          JSON.parse(getCookie("loggedin"))
        );*/
        if (window.loggedin) {
...
rform.addEventListener("submit", function(e) {
  ...
  if (password.value.length < 6) {
    ...
    socket.on("registered", function(user) {
      localStorage.playerseason = "hebrews";
      setCookie("loggedin", "true", 0.08333333333);
      if ("Notification" in window) {
        Notification.requestPermission().then(function() {
          /*new Notification('thank you', {
