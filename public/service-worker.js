...
var jsfiles = [
  ...
  jspath + "log.js",
  jspath + "localsave.js",
  jspath + "leaderboard.js",
  jspath + "scriptureportion2.js",
  jspath + "search3.js",
  jspath + "typequiz3.js",
  jspath + "fade.js",
...
self.addEventListener("fetch", function(evt) {
  if (evt.request.method === "POST"&&(evt.request.url.includes("/setProfileImg")||evt.request.url.includes("complete")||evt.request.url.includes("quote"))) {
    ...
    if (evt.request.url.includes("images/users/")) {
      //console.log("[Service Worker] Fetch (data)", evt.request.url);
      evt.respondWith(
        caches.open(DATA_CACHE_NAME).then(function(cache) {
          return fetch(evt.request)
    ...
    //console.log(evt)
    evt.respondWith(
      fetch(evt.request,{mode: 'no-cors'}).catch(function() {
        return caches.open(CACHE_NAME).then(function(cache) {
...
self.addEventListener("push", function(event) {
  ...
  var options = {
    ...
    sound:
      "https://cdn.glitch.com/cfc340a0-db08-47ab-ae0f-2f9aacd8294c%2Fnotif.mp3",
    /*actions: [
      {
...
function flushQueue() {
  return localforage.getItem("queue").then(function(queue) {
    ...
    console.log("Sending ", queue.length, " requests...", queue);
...
function serialize(request) {
  ...
  try {
    for (
      var _iterator = request.headers.entries()[Symbol.iterator](), _step;
